...
const AssetItemPage: FC = () => {
  ...
  const {data: assetsData} = useQuery({
    queryKey: ['jettonData'],
    queryFn: () => API.getAssetsByWallet(walletAdress),
  })
	...
	return (
		<div className="h-screen bg-gray-800">
          ...
          <div className="mt-3 ml-2 text-center">
            <h1 className="text-4xl   flex justify-center text-white font-semibold">{(state.amount).toFixed(2)} {state.symbol}</h1>
			<span className="text-gray-400 justify-center items-center flex font-light text-sm gap-1"><PiApproximateEqualsBold /> ${(state.amount * state.price).toFixed(2)}</span>
          
						{jettonData.pnl_percentage >= 0 ? (
							<span className=" text-green-600">+{jettonData?.pnl_percentage.toFixed(2)}% (+${jettonData?.pnl_usd})</span>
							) : (
          ...
          <div className="max-w-full">
			...
			<ul className="gap-3 p-7 text-base">
				...
				<li className="flex justify-between mb-5">
					<div className="text-black font-semibold flex items-center ">Average price <MdOutlineInfo className="ml-1 text-gray-500"/></div>
					<div className="font-semibold text-gray-500">${jettonData?.average_price}</div>
				</li>
				<li className="flex justify-between mb-5">
					<div className="text-black font-semibold flex items-center ">Commissions <MdOutlineInfo className="ml-1 text-gray-500"/></div>
					<div className="font-semibold text-gray-500">${jettonData?.commisions}</div>
				</li>
				<li className="flex justify-between mb-5">
					<div className="text-black font-semibold flex items-center ">Market Cap <MdOutlineInfo className="ml-1 text-gray-500"/></div>
