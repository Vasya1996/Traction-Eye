...
export const ProfileItem: FC<ProfileItemProps> = ({ wallet, managed, balance }) => {
  ...
  const handleRemoveClick = () => {
    ...
    if (isConfirmed) {
      ...
      GoogleAnalytics.disconnectWallet();
  ...
  return (
    <li className={`${baseStyles} ${selectedStyles} shadow h-20 p-4 mb-2`}>
      ...
      {managed ? (
        ...
        <span className='font-light'>${ balance % 1 === 0 ?  balance.toFixed(0) : balance.toFixed(2) }</span>
      )}
