...
export function ChartHome({
	...
	const timelineData = useMemo(
		() =>
			getTimelinePeriodAndIntervalKey(Number(userWalletCreationDate), timeline),
		[userWalletCreationDate, timeline]
	);
	...
	const {
		...
		enabled:
			!!targetAddress && !!timelineData?.period && !!timelineData?.interval,
	...
	const currentNetWorth = selectedPoint
		? selectedPoint.netWorth
		: lastChartData?.net_worth;
		...
		<>
			<div className="px-5">
				...
				<div className="flex items-center justify-between">
					<h2
						ref={element1Ref as MutableRefObject<HTMLHeadingElement | null>}
						className={`${downgradeFontSize(
							"text-2xl",
							fontSizeCounter
						)} mb-1 text-white font-bold whitespace-nowrap`}
					>
						${formatIntNumber(Math.round(currentNetWorth ?? 0))}
					</h2>
					<div
						ref={element2Ref as MutableRefObject<HTMLDivElement | null>}
						...
						{currentPnlData && (
							<span
								className={`${downgradeFontSize(
									...
									Number(currentPnlData?.pnl_percentage) >= 0 ||
									!currentPnlData.pnl_percentage
										? "text-green-600"
										: "text-red-600"
								} flex justify-end whitespace-nowrap`}
							>
								{Number(currentPnlData?.pnl_percentage) > 0 ? "+" : ""}
								{formatNumber(currentPnlData.pnl_percentage, false)}% ($
								{formatNumber(currentPnlData.pnl_usd, false)})
							</span>
						)}
						{currentTimestamp && (
							<span
								className={`${downgradeFontSize(
									"text-xs",
									fontSizeCounter
								)} text-gray-400 flex justify-end leading-extra-tight whitespace-nowrap`}
							>
								{getDateAndTime(currentTimestamp)}
							</span>
						)}
					</div>
				</div>
